...
export function DriverOTPVerification() {
  ...
  return (
    <div className="h-full w-full flex flex-col bg-gray-900 overflow-hidden">
      ...
      {/* Content */}
      <div className="flex-1 flex flex-col items-center px-5 pt-8 bg-gray-900">
        {isVerified ? (
          <motion.div
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
          ...
          >
            <div className="w-20 h-20 bg-green-900/30 rounded-full flex items-center justify-center mb-4 border-2 border-green-500">
              <Check className="w-10 h-10 text-green-500" />
            </div>
            <h2 className="text-xl font-bold text-white mb-2">Verified!</h2>
            <p className="text-gray-400 text-sm">Redirecting ka na...</p>
          </motion.div>
        ) : (
          <>
            <p className="text-white font-semibold mb-6">Enter 6-digit code</p>
            ...
            <div className="flex gap-2 mb-8">
              {otp.map((digit, index) => (
                <input
                  ...
                  className="w-12 h-14 text-center text-2xl font-bold bg-gray-800 border-2 border-gray-700 rounded-xl focus:outline-none focus:border-[#F47920] focus:ring-2 focus:ring-[#F47920]/20 text-white"
                />
              ))}
            </div>
